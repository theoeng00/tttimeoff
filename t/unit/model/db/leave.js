...
describe('leave request for two hours', function(){
    var leave = model.Leave.build({
        status : '1',
        date_start : '2015-04-09',
        date_end : '2015-04-09',
    ...
    it('counts as a quarter of a day', function(){
        expect(leave.get_days()[0].get_deducted_days_number()).to.equal(0.25);
    });

    it('does not share a date with another absence without a time slot', function(){
        expect(
            leave.fit_with_leave_request(
                new LeaveRequestParameters({
                    from_date : '2015-04-09',
...
describe('leave request ending with a half day', function(){
    var leave = model.Leave.build({
        status : '1',
    ...
    it('counts full days and the final half day', function(){
        expect(leave.get_days().map(function(day){
            return day.get_deducted_days_number();
        })).to.eql([1, 1, 0.5]);
    });
});
