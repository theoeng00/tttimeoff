...
$(document).ready(function(){
  var fromDayPart = $('select[name="from_date_part"]');
  var toDayPart = $('select[name="to_date_part"]');

  // A one-day request has only one day part. Multi-day requests may use
  // different parts at each end (for example, two full days and one morning).
  fromDayPart.add(toDayPart).on('change', function(){
    var selectedDayPart = $(this).val();
    var isSameDate = $('input.book-leave-from-input').datepicker('getFormattedDate')
      === $('input.book-leave-to-input').datepicker('getFormattedDate');

    if (selectedDayPart === '4' || isSameDate) {
      fromDayPart.val(selectedDayPart);
      toDayPart.val(selectedDayPart);
    }

    if (selectedDayPart === '4') {
