...
const
  ...
  {commentLeave} = require('../comment'),
    Models    = require('../db');
const {createApproval, resolveInitialApprover} = require('../leave_approval');
...
function createNewLeave(args){
  ...
  return Promise
    ...
    .then(() => Models.UserDepartment.findOne({
      where: {user_id: employee.id, is_primary: true},
    }))
    .then(primaryMembership => {
      requestDepartmentId = primaryMembership ? primaryMembership.department_id : employee.DepartmentId;
      return resolveInitialApprover({
        employee,
    ...
    .then(main_supervisor => {
      ...
      initialApprover = main_supervisor;
      ...
      return Promise.resolve(Models.Leave.build({
        ...
        approverId       : main_supervisor ? main_supervisor.id : null,
    ...
    .then(leave => initialApprover ? createApproval({
        leave,
        approverId: initialApprover.id,
        status: leave.is_approved_leave() ? 'approved' : 'pending',
        dbModel: Models,
      }).then(() => leave) : leave)
