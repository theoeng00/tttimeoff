...
CalendarMonth.prototype.is_leave_two_hours = function(day) {
    var leave_day = this._leaves[
        this.get_base_date().date(day).format(this.default_date_format())
    ];
    return leave_day && leave_day.is_two_hours_leave();
};
...
CalendarMonth.prototype.as_for_template = function(){
    ...
    for( var i=1; i<=self.how_many_days(); i++){
        ...
        if ( self.is_weekend(i) ) {
            ...
            if ( self.is_leave_two_hours(i) ) {
              day.is_leave_two_hours = true;

              let leave_type_id = self.get_morning_leave_type_id(i);
              day.morning_leave_type_id = leave_type_id;
              day.leave_color_class_two_hours = leave_type_id && self._leave_types_map[ leave_type_id ]
                ? self._leave_types_map[ leave_type_id ].get_color_class()
                : 'leave_type_color_1';

              leaveLabelForDay.two_hours = `${self._leave_types_map[ leave_type_id ].name || 'leave'} (2 hours)`;
            }
            ...
            if (leaveLabelForDay.two_hours) {
              day.leaveLabel = leaveLabelForDay.two_hours;
            } else if (leaveLabelForDay.morning && (leaveLabelForDay.morning === leaveLabelForDay.afternoon)) {
