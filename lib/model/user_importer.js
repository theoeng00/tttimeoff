...
const add_user_interface_schema = Joi.object().required().keys({
    ...
    receive_approval_report : Joi.boolean().default(false),
    can_view_attendance_reports : Joi.boolean().default(false),
...
function add_user(args) {
  ...
  attributes.receive_approval_report = args.receive_approval_report;
  attributes.can_view_attendance_reports = args.can_view_attendance_reports;
  ...
  return Models.sequelize.transaction(transaction => Promise.resolve()
    ...
    .then(() => Models.User.create(attributes, {transaction}))
    .then(user => Models.UserDepartment.create({
      user_id: user.id,
