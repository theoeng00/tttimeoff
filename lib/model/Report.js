...
const leavesReportCsv = leaves => csv.stringifyAsync([
...
const getUsersWithLeaves = ({
  ...
  if (departmentId) {
    result = result.then(async users => {
      const memberships = await company.sequelize.models.UserDepartment.findAll({
        where: {department_id: departmentId},
        attributes: ['user_id'],
      });
      const memberIds = memberships.map(item => String(item.user_id));
      return users.filter(user => memberIds.includes(String(user.id)) || String(user.DepartmentId) === String(departmentId));
    });
  }
...
const fetchLeavesForLeavesReport = async ({startDate, endDate, departmentId, leaveTypeId, actingUser, dbModel}) => {
  ...
  if (departmentId) {
    const memberships = await dbModel.UserDepartment.findAll({
      where: {department_id: departmentId},
      attributes: ['user_id'],
    });
    const memberIds = memberships.map(item => `${item.user_id}`);
    users = users.filter(user => memberIds.includes(`${user.id}`) || `${user.DepartmentId}` === `${departmentId}`);
  }
