  ...
  let Department = sequelize.define("Department", {
    ...
    is_accrued_allowance : {
      ...
      instanceMethods : {
        ...
        promise_active_users : async function(){
          ...
          const users = await sequelize.models.User.scope('withDepartments').findAll({
            where: {
              $and: [
                sequelize.models.User.get_active_user_filter(),
                {$or: [{id: memberIds}, {DepartmentId: this.id}]},
              ],
            },
          });
        ...
        _promise_team_view : function(args){
          ...
          let
            ...
            has_explicit_range = !! args.end_date;
          ...
          .then(() => {
            ...
            if (moment.utc(start_date).isAfter(moment.utc(end_date), 'day')) {
          ...
          .then(function(users){
            ...
            return Promise.all(
              _.map(
                users,
                function(user){
                  return user.getMyActiveLeavesForDateRange({
                    dateStart : moment.utc(start_date).startOf('month'),
                    dateEnd : moment.utc(end_date).endOf('month'),
                  })
          ...
          return Promise.join(
            ...
            function(company, users_and_leaves){
              ...
              let number_of_months = moment.utc(end_date).startOf('month')
                .diff(moment.utc(start_date).startOf('month'), 'months');
              ...
              users_and_leaves.forEach( user_data => {
                ...
                if (has_explicit_range) {
                  user_data.days = user_data.days.filter(day => day.moment.isBetween(start_date, end_date, 'day', '[]'));
                }
              });
