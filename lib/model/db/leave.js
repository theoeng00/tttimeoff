    ...
    var Leave = sequelize.define("Leave", {
      ...
      classMethods : {
        associate : function( models ){
          ...
          Leave.belongsTo(models.Department, {as: 'request_department', foreignKey: 'request_department_id'});
          Leave.belongsTo(models.LeaveType, { as : 'leave_type' } );
          Leave.hasMany(models.LeaveApproval, {as: 'approvals', foreignKey: 'leave_id'});
...
does_start_two_hours : function() {
  return this.day_part_start === Leave.leave_day_part_two_hours();
},
...
does_end_two_hours : function() {
  return this.day_part_end === Leave.leave_day_part_two_hours();
},
...
get_deducted_days_number : function(args) {
  return _.reduce(
    this.get_deducted_days(args),
    function(total, leave_day) {
      return total + leave_day.get_deducted_days_number();
    },
    0
  );
},
...
get_deducted_days : function(args) {
  ...
  if (args && args.hasOwnProperty('year')) {
    year = cutoffPeriod.forYear(args.year);
  }
  ...
  leave_days = _.filter(
    _.map(this.get_days(), function(leave_day){
      ...
      if (year && !moment.utc(leave_day.date).isBetween(year.start, year.end, null, '[]')) return;
...
promise_to_reject : function(args) {
  ...
  return self.save().then(() => require('../leave_approval').markDecision({
    leave: self,
    approverId: by_user.id,
    status: 'rejected',
    dbModel: sequelize.models,
  })).then(() => self);
},
