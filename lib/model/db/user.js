...
module.exports = function(sequelize, DataTypes) {
  ...
  var User = sequelize.define("User", {
      ...
      receive_approval_report : {
        ...
        allowNull    : false,
        defaultValue : false,
        comment      : 'Indicate if user receives CSV snapshots after leave approvals',
      },
      can_view_attendance_reports : {
        type         : DataTypes.BOOLEAN,
        allowNull    : false,
        defaultValue : false,
        comment      : 'Indicate if user can view company-wide attendance and OT reports',
      },
      start_date : {
          type         : DataTypes.DATE,
          allowNull    : false,
  ...
  return {
    ...
    promise_users_I_can_manage : async function(){
      ...
      if ( self.is_admin() ) {
        ...
        users = await self.promise_supervised_users();
      }
     ...
     *
     * */
    promise_supervisors : async function(){
      const memberships = await sequelize.models.UserDepartment.findAll({
        where: {user_id: this.id},
        include: [{model: sequelize.models.User, as: 'manager'}],
      });
      const membershipManagers = memberships.map(item => item.manager).filter(Boolean);

      const department = await this.getDepartment({scope : ['with_boss', 'with_supervisors']});
      const legacyManagers = department ? _.flatten([department.boss, department.supervisors]) : [];

      return _.uniq(membershipManagers.concat(legacyManagers).filter(manager =>
        manager && String(manager.id) !== String(this.id)
      ), manager => manager.id);
    },

    promise_department_memberships : function(){
      return sequelize.models.UserDepartment.findAll({
        where: {user_id: this.id},
        include: [{
    ...
    promise_departments : async function(){
      const departments = await this.getDepartments();
      const legacyDepartment = await this.getDepartment();
      return _.uniq(departments.concat(legacyDepartment ? [legacyDepartment] : []), department => department.id);
    },
    ...
    promise_supervised_departments : function() {
      ...
      return Promise.join(
        sequelize.models.DepartmentSupervisor.findAll({where: {user_id: self.id}}),
        sequelize.models.UserDepartment.findAll({where: {manager_user_id: self.id}}),
        (departmentSupervisors, managedMemberships) => departmentSupervisors
          .map(item => item.department_id)
          .concat(managedMemberships.map(item => item.department_id))
      )
    ...
    promise_supervised_users : async function () {
      const departments = await this.promise_supervised_departments();
      const memberships = await sequelize.models.UserDepartment.findAll({
        where: {manager_user_id: this.id},
      });
      const membershipUserIds = memberships.map(item => item.user_id);

      const users = await this.Model.findAll({
        where: {
          $or: [
            {id: membershipUserIds},
            {DepartmentId: departments.map(department => department.id)},
          ],
        },
      });

      return _.uniq(users, user => user.id);
    },
    ...
    remove : function() {
      ...
      // Supervisor links are optional. Clear them instead of forcing an admin
      // to reassign every relationship before removing the user.
      return sequelize.models.Leave.findAll({
        ...
        .then(pendingLeaves => Promise.all([
          sequelize.models.Department.update(
            {bossId: null},
            {where: {bossId: self.id}}
          ),
          sequelize.models.UserDepartment.update(
            {manager_user_id: null},
            {where: {manager_user_id: self.id}}
          ),
          sequelize.models.DepartmentSupervisor.destroy({
            where: {user_id: self.id},
          }),
          sequelize.models.Leave.update(
            {approverId: null},
            {where: {id: pendingLeaves.map(leave => leave.id)}}
          ),
          sequelize.models.LeaveApproval.update(
            {status: 'unassigned', decided_at: new Date()},
            {where: {leave_id: pendingLeaves.map(leave => leave.id), status: 'pending'}}
          ),
        ]))
        .then(() => self.getMy_leaves())
        .then(function(leaves){
          // remove all leaves
          return Promise.all(
            _.map(leaves, function(leave){
              return sequelize.models.LeaveApproval.destroy({where: {leave_id: leave.id}})
                .then(() => leave.destroy());
            })
          );
        })
        ...
        .then(function(){
          return Promise.all([
            sequelize.models.UserDepartment.destroy({where: {user_id: self.id}}),
            sequelize.models.AttendanceCorrectionRequest.destroy({where: {$or: [{user_id: self.id}, {approver_user_id: self.id}]}}),
            sequelize.models.AttendanceClockOutCorrectionRequest.destroy({where: {$or: [{user_id: self.id}, {approver_user_id: self.id}]}}),
          ]).then(() => sequelize.models.Attendance.destroy({where: {user_id: self.id}}));
        })
  ...
  return {
    ...
    register_new_admin_user : function(attributes){
      ...
      return sequelize.models.User.find_by_email( attributes.email )
        ...
        .then(function(){
          return Promise.all(new_departments.map(function(department, index){
            return sequelize.models.UserDepartment.create({
              user_id: new_user.id,
...
function withAssociations() {
  ...
  this.associate = function(models){
    ...
    models.User.hasMany(models.UserDepartment, {
      as: 'department_memberships',
      foreignKey: 'user_id',
    });
    models.User.belongsToMany(models.Department, {
      as: 'departments',
      foreignKey: 'user_id',
      otherKey: 'department_id',
      through: models.UserDepartment,
    });
    ...
    models.User.hasMany(models.Attendance, {as: 'attendance_records', foreignKey: 'user_id'});
  };
}
