...
TeamView.prototype.promise_team_view_details = function(args){
  ...
  if (user.is_admin() || user.can_view_attendance_reports || user.company.share_all_absences) {
    ...
    promise_departments = Promise.join(
      user.promise_departments(),
      user.promise_supervised_departments(),
      function(memberDepartments, supervisedDepartments) {
        return normalise_departments_func(null, supervisedDepartments.concat(memberDepartments));
      }
    );
  }
    ...
    .forEach(node => {
      ...
      node
        ...
        .forEach( day => {
          ...
          if (day.is_leave_two_hours) {

            const leaveTypeId = day.morning_leave_type_id;
            leave_type_stat[ leaveTypeId ] = leave_type_stat[ leaveTypeId ] + 0.25;

            if (leave_types_map[ leaveTypeId ] && leave_types_map[ leaveTypeId ].use_allowance) {
              deducted_days = deducted_days + 0.25;
            }
          } else if (day.is_leave_morning) {
