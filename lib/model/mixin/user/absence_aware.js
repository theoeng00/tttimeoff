...
module.exports = function(sequelize){
  ...
  this.promise_leaves_to_be_processed = async function(){
    let self = this;
    let approverFilter = {approverId: self.id};

    if (self.is_admin()) {
      const companyUsers = await sequelize.models.User.findAll({
      ...
      approverFilter = {
        $or: [
          {approverId: self.id},
          {approverId: null, userId: companyUsers.map(user => user.id)},
        ],
      };
    }
  ...
  this.promise_adjustment_and_carry_over_for_year = function(year){
    ...
    year = year === undefined ? cutoffPeriod.year(moment.utc()).year : year;
    year = String(moment.isMoment(year) ? cutoffPeriod.year(year).year : year);
  ...
  this.promise_to_update_adjustment = function(args) {
    let
      self = this,
      year = args.year || cutoffPeriod.year(moment.utc()).year,
  ...
  this.validate_leave_fits_into_remaining_allowance = function(args){
    ...
    .then(function(args){
      ...
      let deducted_days =  leave.get_deducted_days_number({
          year       : cutoffPeriod.year(year).year,
          user       : employee,
          leave_type : leave_type,
        });
