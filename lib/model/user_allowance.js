...
class UserAllowance {
  ...
  get number_of_days_available_in_allowance() {
    ...
    if (self.user.start_date && cutoffPeriod.year(self.user.start_date).year > cutoffPeriod.year(self._now).year) {
      return 0;
    }
  ...
  get employement_range_adjustment() {
    const self = this;
    const period = cutoffPeriod.year(self._now);
    const startDate = moment.max(moment.utc(self.user.start_date), period.start);
    const endDate = self.user.end_date ? moment.min(moment.utc(self.user.end_date), period.end) : period.end;

    if (moment.utc(self.user.start_date).isAfter(period.end, 'day')) return 0;
    if (startDate.isAfter(endDate, 'day')) return -self.nominal_allowance;
    if (startDate.isSame(period.start, 'day') && endDate.isSame(period.end, 'day')) return 0;

    const daysInPeriod = period.end.clone().startOf('day').diff(period.start, 'days');
    const employedDays = endDate.clone().startOf('day').diff(startDate.clone().startOf('day'), 'days');

    return -(self.nominal_allowance - Math.round(self.nominal_allowance * employedDays / daysInPeriod));
  }
  ...
  get accrued_adjustment() {
    ...
    const period = cutoffPeriod.year(now);
    ...
    const period_starts_at = moment.max(moment.utc(self.user.start_date), period.start);
    const period_ends_at = self.user.end_date ? moment.min(moment.utc(self.user.end_date), period.end) : period.end;

    if (period_starts_at.isAfter(period_ends_at, 'day')) return -allowance;

    let days_in_period = period_ends_at.diff(period_starts_at, 'days');

    let remaining_days = Math.max(0, period_ends_at.diff(moment.max(now, period_starts_at), 'days'));
    let delta = allowance * remaining_days / days_in_period;
  ...
  static promise_allowance(args) {
    ...
    let
      user = args.user,
      year = args.year,
      allowancePeriod = cutoffPeriod.year(year),
    ...
    flow = flow.then(() => user.promise_adjustment_and_carry_over_for_year(allowancePeriod.year));
    ...
    flow = flow.then(() => Promise.resolve(
      number_of_days_taken_from_allowance = user.calculate_number_of_days_taken_from_allowance({year : allowancePeriod.year})
    ));
    ...
    flow = flow.then(() => {
      ...
      if (forceNow && now) {
        args.now = now;
      } else if (allowancePeriod.year !== cutoffPeriod.year(moment.utc()).year) {
        args.now = allowancePeriod.year < cutoffPeriod.year(moment.utc()).year
          ? allowancePeriod.end
          : allowancePeriod.start;
      }
