...
LeaveRequest.prototype._does_fit_with_point = function(leave_day, point_name){
    var requested_part = String(this[point_name+'_part']);

    if (moment.utc(leave_day.date).format('YYYY-MM-DD') !== moment.utc(this[point_name]).format('YYYY-MM-DD')) {
      return false;
    }

    // Only complementary morning and afternoon requests can share a date.
    // A two-hour request does not identify a time slot, so it cannot safely be
    // combined with another absence on the same day.
    return (
      (leave_day.is_morning_leave() && requested_part === String(model.Leave.leave_day_part_afternoon()))
      ||
      (leave_day.is_afternoon_leave() && requested_part === String(model.Leave.leave_day_part_morning()))
    );
};
