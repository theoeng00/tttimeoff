...
const {escalate, getEscalationState} = require('../model/leave_approval');
...
router.get('/', function(req, res){
  ...
  Promise.join(
      req.user
        ...
        .then(leaves => Promise.map(leaves, async leave => {
          const state = await getEscalationState({leave, dbModel});
          leave.can_escalate = state.canEscalate;
          leave.next_approver = state.nextApprover;
          return leave;
        }))
...
router.post('/escalate/', async function(req, res) {
  const requestId = validator.trim(req.body.request || '');
  const dbModel = req.app.get('db_model');

  try {
    if (!validator.isNumeric(requestId)) throw new Error('Invalid leave request');

    const leave = await dbModel.Leave.findOne({
      where: {id: requestId, userId: req.user.id},
      include: [
        {model: dbModel.User, as: 'user'},
    ...
    const result = await escalate({leave, requestedBy: req.user, dbModel});
    const previousApprover = result.previousApprover || leave.approver;
    leave.approver = result.nextApprover;

    await (new EmailTransport()).promise_leave_escalated_email({
      leave,
      previousApprover,
      nextApprover: result.nextApprover,
    });

    req.session.flash_message(`Request was escalated to ${result.nextApprover.full_name()}`);
  } catch (error) {
    console.error(`Failed to escalate leave request ${requestId} for user ${req.user.id}: ${error.stack}`);
