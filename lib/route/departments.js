...
function get_and_validate_department(args) {
  ...
  // current company
  if (boss_id && !validator.isNumeric( boss_id ) ) {
    req.session.flash_error(
      'New boss reference for '+department_name+' should be numeric'
    );
  } else if (boss_id && ! _.contains(
...
router.post('/departments/delete/:department_id/', function(req, res){
  ...
  .then(function(departments){
    ...
    return req.app.get('db_model').UserDepartment.count({
      where: {department_id: department_to_remove.id},
    }).then(function(membershipCount){
      if (membershipCount > 0) {
        req.session.flash_error(
          'Cannot remove department '+department_to_remove.name
            +' as it still has '+membershipCount+' members.'
        );
        throw new Error('Department still has members');
      }

      return req.app.get('db_model').DepartmentSupervisor.destroy({
        where: {department_id: department_to_remove.id},
      }).then(() => department_to_remove.destroy());
    });
  })
  .then(function(){
