...
var validator = require('validator'),
    ...
    cutoffPeriod = require('../../util/cutoff_period');
...
module.exports = function(args){
    ...
    if (  !validator.matches(from_date_part, /^[1234]$/)
       || !validator.matches(to_date_part, /^[1234]$/)
     ){
    ...
    if (from_date === to_date && from_date_part !== to_date_part) {
      req.session.flash_error('A one-day absence must use the same day part');
    }

    // A two-hour absence has no time-of-day selection. It therefore occupies
    // the whole date for booking purposes and must be a single-day request.
    if (from_date_part === '4' || to_date_part === '4') {
      if (from_date !== to_date || from_date_part !== '4' || to_date_part !== '4') {
        req.session.flash_error('Two-hour absence must be requested for one day only');
      }
    }

    if (cutoffPeriod.year(from_date).year !== cutoffPeriod.year(to_date).year) {
        req.session.flash_error('Current implementation does not allow leave across the 20 December cutoff. Please split your request into two parts');
    }
