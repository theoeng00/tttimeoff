...
router.post('/add/', function(req, res){
  ...
  .then(() => UserImporter.add_user({
    ...
    receive_approval_report : new_user_attributes.receive_approval_report,
    can_view_attendance_reports : new_user_attributes.can_view_attendance_reports,
  ...
  .catch(function(error){
    ...
    if (created_user) {
      req.session.flash_error(
        'User account was added, but the welcome email could not be prepared or sent.'
      );
      return res.redirect_with_session('../');
    }
...
router.get('/edit/:user_id/', function(req, res){
  ...
  .then(function(company){
    ...
    return Promise.all([
      employee.promise_schedule_I_obey(),
      employee.promise_department_memberships(),
      req.app.get('db_model').User.findAll({
      ...
      .then(function(results){
        ...
        const companyUsers = results[2];
        company.departments.forEach(department => {
          const membership = memberships.find(item => String(item.department_id) === String(department.id));
          department.is_member = Boolean(membership) || String(department.id) === String(employee.DepartmentId);
          department.manager_options = companyUsers
            .filter(user => String(user.id) !== String(employee.id))
            .map(user => ({
              id: user.id,
              name: user.name,
              lastname: user.lastname,
              selected: Boolean(membership && String(membership.manager_user_id) === String(user.id)),
            }));
        });
...
router.get('/edit/:user_id/absences/', function(req, res){
  let
    ...
    user_allowance,
    allowanceYear = cutoffPeriod.year(moment.utc()).year;
  ...
  .then(employee => Promise.join(
    ...
    employee
      .promise_adjustmet_for_year(allowanceYear),

    employee
      .promise_carried_over_allowance_for_year(allowanceYear),
...
async function syncDepartmentMemberships({req, employee, departments, model}) {
  const selectedIds = [].concat(req.body.department_ids || []).map(String);
  const primaryDepartmentId = String(employee.DepartmentId);
  if (!selectedIds.includes(primaryDepartmentId)) selectedIds.push(primaryDepartmentId);

  const allowedDepartments = departments.filter(department => selectedIds.includes(String(department.id)));
  if (allowedDepartments.length !== selectedIds.length) throw new Error('Invalid department membership');

  const companyUsers = await model.User.findAll({
    where: {companyId: employee.companyId},
    attributes: ['id'],
    raw: true,
  });
  const allowedUserIds = companyUsers.map(user => String(user.id));

  return model.sequelize.transaction(async transaction => {
    const currentMemberships = await model.UserDepartment.findAll({
      where: {department_id: allowedDepartments.map(department => department.id)},
      transaction,
    });

    await model.UserDepartment.destroy({
      where: {
        user_id: employee.id,
        department_id: {$notIn: selectedIds},
      },
      transaction,
    });

    for (const department of allowedDepartments) {
      const submittedManagerId = req.body[`manager_${department.id}`];
      const managerId = submittedManagerId || null;

      if (managerId && (!allowedUserIds.includes(String(managerId)) || String(managerId) === String(employee.id))) {
        throw new Error('Invalid department manager');
      }

      const managerByUserId = currentMemberships
        .filter(item => String(item.department_id) === String(department.id))
        .reduce((result, item) => {
          result[String(item.user_id)] = item.manager_user_id;
          return result;
        }, {});
      managerByUserId[String(employee.id)] = managerId;
      const seen = new Set([String(employee.id)]);
      let managerToCheck = managerId;
      while (managerToCheck) {
        if (seen.has(String(managerToCheck))) throw new Error(`Reporting line cycle in ${department.name}`);
        seen.add(String(managerToCheck));
        managerToCheck = managerByUserId[String(managerToCheck)];
      }

      const values = {
        manager_user_id: managerId,
        is_primary: String(department.id) === primaryDepartmentId,
      };
      const existing = await model.UserDepartment.findOne({
        where: {user_id: employee.id, department_id: department.id},
        transaction,
      });

      if (existing) await existing.updateAttributes(values, {transaction});
      else await model.UserDepartment.create(Object.assign({
        user_id: employee.id,
...
router.post('/edit/:user_id/', function(req, res){
  ...
  .then(function(loadedCompany){
    ...
    new_user_attributes = get_and_validate_user_parameters({
      req         : req,
      item_name   : 'user',
      departments : loadedCompany.departments,
    });
    ...
    employee = loadedCompany.users[0];
  ...
  .then(function(){
    ...
    return employee
      ...
      .then(() => syncDepartmentMemberships({
        req,
        employee,
        departments: company.departments,
        model,
      ...
      .then(() => {
        if ( adjustment !== undefined  ) {
          return employee.promise_to_update_adjustment({
            year : cutoffPeriod.year(moment.utc()).year,
...
router.get('/', function(req, res) {
    ...
    var department_id = req.query['department'],
        users_filter = {},
        model = req.app.get('db_model'),
        allowancePeriod = cutoffPeriod.year(moment.utc());
    ...
    .then(async function(){
      ...
      const memberships = await model.UserDepartment.findAll({
        where: {department_id},
        attributes: ['user_id'],
      });
      users_filter = {
        $or: [
          {id: memberships.map(item => item.user_id)},
            ...
            // taken from allowance
            {
              ...
              where : {
                ...
                $or : {
                  date_start : {
                    $between : [
                      allowancePeriod.start.format('YYYY-MM-DD'),
                      allowancePeriod.end.format('YYYY-MM-DD HH:mm'),
                    ]
                  },
                  date_end : {
                    $between : [
...
function get_and_validate_user_parameters(args) {
    ...
    var name     = validator.trim(req.body['name']),
        ...
        receive_approval_report = validator.toBoolean(req.body['receive_approval_report']),
        can_view_attendance_reports = validator.toBoolean(req.body['can_view_attendance_reports']);
    ...
    var attributes = {
        ...
        receive_approval_report : receive_approval_report,
