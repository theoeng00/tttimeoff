...
const clockOutCorrectionLogic = require('../model/attendance_clock_out_correction_request');
...
async function sendAttendanceCorrectionEmail(args) {
  try {
    await (new EmailTransport()).promise_attendance_correction_email(args);
  } catch (error) {
    console.error('Attendance correction was saved but email notification failed: %s', error.message);
  }
}
...
router.post('/corrections/', async function(req, res) {
  ...
  try {
    ...
    await sendAttendanceCorrectionEmail({
      recipient: approver,
      ...
      context: {
        ...
        work_date: moment.utc(attendance.work_date).format('DD/MM/YYYY'),
        original_time: moment(attendance.clock_in_at).tz(company.timezone).format('HH:mm'),
        requested_time: moment(values.requested_clock_in_at).tz(company.timezone).format('HH:mm'),
...
async function decideAttendanceCorrection(req, action) {
  ...
  const request = await db.sequelize.transaction(async transaction => {
  ...
  const requester = await db.User.findById(request.user_id);
  if (requester) await sendAttendanceCorrectionEmail({
    recipient: requester,
    ...
    context: {
      ...
      work_date: moment.utc(request.work_date).format('DD/MM/YYYY'),
      requested_time: moment(request.requested_clock_in_at).tz(company.timezone).format('HH:mm'),
...
router.get('/clock-out-corrections/', async function(req, res, next) {
  const db = req.app.get('db_model');
  try {
    const company = await req.user.getCompany();
    const now = attendanceLogic.companyMoment(company);
    const earliestDate = now.clone().subtract(6, 'days').format('YYYY-MM-DD');
    const hasReportAccess = canViewAttendanceReports(req.user);
    const where = hasReportAccess
      ? {company_id: company.id}
      : {company_id: company.id, $or: [{user_id: req.user.id}, {approver_user_id: req.user.id}]};
    const requests = await db.AttendanceClockOutCorrectionRequest.findAll({
      where,
      include: [{model: db.User, as: 'user'}, {model: db.User, as: 'approver'}],
      order: [['created_at', 'DESC']],
    });
    const pendingAttendanceIds = requests
      .filter(item => item.status === 'pending' && String(item.user_id) === String(req.user.id))
      .map(item => String(item.attendance_id));
    const attendances = req.user.admin ? [] : await db.Attendance.findAll({
      where: {user_id: req.user.id, work_date: {$gte: earliestDate, $lte: now.format('YYYY-MM-DD')}},
      order: [['work_date', 'DESC']],
    });
    const candidates = attendances.filter(item => !pendingAttendanceIds.includes(String(item.id)));

    candidates.forEach(item => {
      const clockIn = moment(item.clock_in_at).tz(company.timezone).format('HH:mm');
      const clockOut = item.clock_out_at ? moment(item.clock_out_at).tz(company.timezone).format('HH:mm') : '—';
      item.optionLabel = `${moment.utc(item.work_date).format('DD/MM/YYYY')} · Clock-in ${clockIn} · Clock-out ${clockOut}`;
    });
    requests.forEach(item => {
      item.statusLabel = req.__(item.status.charAt(0).toUpperCase() + item.status.slice(1));
      item.originalClockOutTime = item.original_clock_out_at ? moment(item.original_clock_out_at).tz(company.timezone).format('HH:mm') : '—';
      item.requestedClockOutTime = moment(item.requested_clock_out_at).tz(company.timezone).format('HH:mm');
      item.canDecide = item.status === 'pending' && String(item.approver_user_id) === String(req.user.id);
    });

    res.render('attendance_clock_out_corrections', {
      requests,
      candidates,
      canSubmit: candidates.length > 0,
      title: req.__('Clock-out correction requests'),
    });
  } catch (error) { next(error); }
});

router.post('/clock-out-corrections/', async function(req, res) {
  const db = req.app.get('db_model');
  try {
    if (req.user.admin) throw Object.assign(new Error('System administrator is not an employee'), {user_message: 'บัญชีผู้ดูแลระบบไม่สามารถขอแก้เวลาออกงานได้'});
    if (!/^\d+$/.test(String(req.body.attendance_id))) throw Object.assign(new Error('Invalid attendance id'), {user_message: 'กรุณาเลือกรายการลงเวลา'});
    const company = await req.user.getCompany();
    const now = attendanceLogic.companyMoment(company);
    const attendance = await db.Attendance.findOne({
      where: {
        ...
        user_id: req.user.id,
        work_date: {$gte: now.clone().subtract(6, 'days').format('YYYY-MM-DD'), $lte: now.format('YYYY-MM-DD')},
      },
    });
    if (!attendance) throw Object.assign(new Error('Attendance is unavailable'), {user_message: 'แก้ไขได้เฉพาะรายการลงเวลาย้อนหลังไม่เกิน 7 วัน'});
    const duplicate = await db.AttendanceClockOutCorrectionRequest.findOne({where: {attendance_id: attendance.id, user_id: req.user.id, status: 'pending'}});
    if (duplicate) throw Object.assign(new Error('Pending Clock-out correction exists'), {user_message: 'รายการนี้มีคำขอแก้เวลาออกงานที่รออนุมัติอยู่แล้ว'});
    const values = clockOutCorrectionLogic.validateRequest(req.body, attendance, company.timezone, now.toDate());
    const approver = await resolveInitialApprover({employee: req.user, departmentId: req.user.DepartmentId, dbModel: db});
    if (!approver || String(approver.id) === String(req.user.id)) {
      throw Object.assign(new Error('No Clock-out correction approver configured'), {user_message: 'ยังไม่ได้กำหนดหัวหน้าผู้อนุมัติ'});
    }
    await db.AttendanceClockOutCorrectionRequest.create(Object.assign(values, {
      attendance_id: attendance.id,
      company_id: company.id,
      user_id: req.user.id,
      approver_user_id: approver.id,
      work_date: attendance.work_date,
      original_clock_out_at: attendance.clock_out_at,
      status: 'pending',
    }));
    await sendAttendanceCorrectionEmail({
      recipient: approver,
      template_name: 'attendance_correction_request',
      path: '/attendance/clock-out-corrections/',
      context: {
        correction_type: 'Clock-out',
        requester_name: req.user.full_name(),
        approver_name: approver.full_name(),
        work_date: moment.utc(attendance.work_date).format('DD/MM/YYYY'),
        original_time: attendance.clock_out_at ? moment(attendance.clock_out_at).tz(company.timezone).format('HH:mm') : '—',
        requested_time: moment(values.requested_clock_out_at).tz(company.timezone).format('HH:mm'),
        reason: values.reason,
      },
    });
    req.session.flash_message('ส่งคำขอแก้เวลาออกงานให้หัวหน้าอนุมัติแล้ว');
  } catch (error) {
    console.error('Failed to create Clock-out correction for user %s: %s', req.user.id, error.message);
    req.session.flash_error(error.user_message || 'ส่งคำขอแก้เวลาออกงานไม่สำเร็จ');
  }
  res.redirect_with_session('/attendance/clock-out-corrections/');
});

async function decideClockOutCorrection(req, action) {
  const db = req.app.get('db_model');
  const company = await req.user.getCompany();
  if (!/^\d+$/.test(String(req.params.id))) throw Object.assign(new Error('Invalid Clock-out correction id'), {user_message: 'ไม่พบคำขอแก้เวลาออกงาน'});

  const request = await db.sequelize.transaction(async transaction => {
    const request = await db.AttendanceClockOutCorrectionRequest.findOne({
      where: {id: req.params.id, company_id: company.id, approver_user_id: req.user.id, status: 'pending'},
      transaction,
      lock: transaction.LOCK.UPDATE,
    });
    if (!request) throw Object.assign(new Error('Clock-out correction is unavailable'), {user_message: 'คำขอนี้ถูกดำเนินการแล้วหรือคุณไม่มีสิทธิ์อนุมัติ'});
    ...
    if (action === 'approve') {
      ...
      if (!attendance) throw Object.assign(new Error('Attendance record is missing'), {user_message: 'ไม่พบรายการ Clock-out ที่ต้องการแก้ไข'});
      const result = attendanceLogic.clockOutResult(company, request.requested_clock_out_at);
      await attendance.updateAttributes({
    ...
    const updated = await db.AttendanceClockOutCorrectionRequest.update({
      status: action === 'approve' ? 'approved' : 'rejected',
      decided_at: new Date(),
    }, {where: {id: request.id, status: 'pending'}, transaction});
    if (!updated[0]) throw Object.assign(new Error('Clock-out correction was already decided'), {user_message: 'คำขอนี้ถูกดำเนินการแล้ว'});
    return request;
  });
  ...
  if (requester) await sendAttendanceCorrectionEmail({
    ...
    context: {
      ...
      approver_name: req.user.full_name(),
      work_date: moment.utc(request.work_date).format('DD/MM/YYYY'),
      requested_time: moment(request.requested_clock_out_at).tz(company.timezone).format('HH:mm'),
      decision: action === 'approve' ? 'approved' : 'rejected',
    },
  });
}

router.post('/clock-out-corrections/:id/approve/', async function(req, res) {
  try {
    await decideClockOutCorrection(req, 'approve');
    req.session.flash_message('อนุมัติการแก้เวลาออกงานแล้ว');
  } catch (error) {
    console.error('Failed to approve Clock-out correction %s: %s', req.params.id, error.message);
    req.session.flash_error(error.user_message || 'อนุมัติคำขอไม่สำเร็จ');
  }
  res.redirect_with_session('/attendance/clock-out-corrections/');
});

router.post('/clock-out-corrections/:id/reject/', async function(req, res) {
  try {
    await decideClockOutCorrection(req, 'reject');
    req.session.flash_message('ปฏิเสธการแก้เวลาออกงานแล้ว');
  } catch (error) {
    console.error('Failed to reject Clock-out correction %s: %s', req.params.id, error.message);
    req.session.flash_error(error.user_message || 'ปฏิเสธคำขอไม่สำเร็จ');
  }
  res.redirect_with_session('/attendance/clock-out-corrections/');
});
