...
async function pageState(req) {
  ...
  const attendance = await req.app.get('db_model').Attendance.findOne({
    where: {company_id: company.id, user_id: req.user.id, work_date: workDate},
  });
  ...
  const verificationReady = company.attendance_verification_mode === 'office_network'
    ? attendanceLogic.isAddressInCidr(String(company.attendance_network_cidr).split('/')[0], company.attendance_network_cidr)
    : locationConfigured(company);
  return {
    company,
    attendance,
    isSystemAdmin: req.user.admin,
    canViewAttendanceReports: canViewAttendanceReports(req.user),
    verificationIsNetwork: company.attendance_verification_mode === 'office_network',
    isWorkingDay,
...
router.post('/clock-in/', async function(req, res) {
  ...
  try {
    ...
    const proof = attendanceLogic.verifyAttendanceRequest(company, req.body, remoteAddress(req));
...
router.post('/clock-out/', async function(req, res) {
  ...
  try {
    ...
    const record = await db.Attendance.findOne({where: {company_id: company.id, user_id: req.user.id, work_date: workDate}});
...
router.post('/settings/', ensureAdmin, async function(req, res) {
  try {
    ...
    const verificationMode = req.body.attendance_verification_mode;
    const networkCidr = String(req.body.attendance_network_cidr || '').trim();
    ...
    if (!['gps', 'office_network'].includes(verificationMode)) throw new Error('วิธีตรวจสอบการลงเวลาไม่ถูกต้อง');
    if (enabled && verificationMode === 'gps' && (latitude === null || longitude === null)) throw new Error('กรุณาระบุพิกัดสำนักงานก่อนเปิดระบบ');
    if (verificationMode === 'office_network' && !attendanceLogic.isAddressInCidr(networkCidr.split('/')[0], networkCidr)) throw new Error('วงเครือข่ายออฟฟิศไม่ถูกต้อง');
    ...
    await company.updateAttributes({
      ...
      attendance_verification_mode: verificationMode,
      attendance_network_cidr: networkCidr,
