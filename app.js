...
app.use(function(req,res,next){
  ...
  var current_lang = req.query.lang || (req.session && req.session.lang) || req.cookies.lang || 'en';
  if (!['en', 'th'].includes(current_lang)) {
    current_lang = 'en';
  }

  if (req.query.lang && ['en', 'th'].includes(req.query.lang)) {
    res.cookie('lang', req.query.lang, { maxAge: 900000000, httpOnly: false });
    if (req.session) {
      req.session.lang = req.query.lang;
    }
  }

  req.setLocale(current_lang);
  res.locals.current_lang = current_lang;
  res.locals.is_th = (current_lang === 'th');
  res.locals.is_en = (current_lang === 'en');
  ...
  res.locals.database_dialect = app.get('db_model').sequelize.getDialect();
...
app.get('/lang/:locale', function(req, res) {
  ...
  if (['en', 'th'].includes(locale)) {
    res.cookie('lang', locale, { maxAge: 900000000, httpOnly: false });
    if (req.session) {
...
app.use(
  '/backup/',
  require('./lib/route/backup')
);

app.use(
  '/restore/',
  require('./lib/route/restore')
);
